...
<table class="table table-hover table-striped">
    ...
    <tbody>
        <?php
        include "koneksi.php";
        $qry_histori=mysqli_query($conn,"select * from peminjaman_buku order by id_peminjaman desc");
        $no=0;
        while($dt_histori=mysqli_fetch_array($qry_histori)){
            $no++;
            //menampilkan buku yang dipinjam
            $buku_dipinjam="<ol>";
            $qry_buku=mysqli_query($conn,"select * from  detail_peminjaman_buku join buku on buku.id_buku=detail_peminjaman_buku.id_buku
            join peminjaman_buku on peminjaman_buku.id_peminjaman=detail_peminjaman_buku.id_peminjaman_buku where peminjaman_buku.id_peminjaman = '".$dt_histori['id_peminjaman']."'");
            ...
            if(mysqli_num_rows($qry_cek_kembali)>0){
                ...
                $button_kembali="<a href='kembali.php?id=".$dt_histori['id_peminjaman']."' class='btn btn-warning' onclick='return confirm(\"Terimakasih telah mengembalikan\")'>Kembalikan</a>";
            }
        ?>
            <tr>
